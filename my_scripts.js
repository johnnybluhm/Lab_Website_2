...
*/
	function loadStatsPage(){
		...
		var losses=0;
		for (var i=2;i<14;i++){
			var homeScore= parseInt(document.getElementById("stats_table").rows[i].cells[2].innerHTML,10);
			var awayScore= parseInt(document.getElementById("stats_table").rows[i].cells[3].innerHTML,10);
			var awayTeam=document.getElementById("stats_table").rows[i].cells[1].innerHTML;


			if (homeScore>awayScore){
				wins++;
				document.getElementById("stats_table").rows[i].cells[4].innerHTML="Colorado";

			}
			else{
				losses++;
				document.getElementById("stats_table").rows[i].cells[4].innerHTML=awayTeam;
			}
		}//end for
...
function loadPlayersPage()
{
  ...
  myAnchor.onclick=switchPlayers(i);
  var name = players[i].name;
  myAnchor.innerHTML=name
  document.getElementById("player_selector").appendChild(myAnchor);
}
}
